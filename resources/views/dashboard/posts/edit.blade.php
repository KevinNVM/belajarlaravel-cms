        ...
        <form method="POST" action="/dashboard/posts/{{ $post->slug }}" class="mb-5" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control bg-transparent @error('slug') is-invalid @enderror" id="slug" name="slug"
                    value="{{ old('slug', $post->slug) }}" placeholder="Enter Post Title First" readonly>
                @error('slug')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            <div class="mb-3">
                ...
                <select class="form-select" name="category_id">
                    @foreach ($categories as $cat)
                        <option value="{{ $cat->id }}" {{ old('category_id', $post->cateory_id) == $cat->id ? ' selected' : ' ' }}>
                            {{ $cat->name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-3">
                ...
                <img class="img-preview img-fluid mb-3 col-sm-5"
                    {{ ($post->image) ? 'src=' . url("storage/$post->image") : "" }}>
